...
function Atom(group, name, element, coordinates) {
	...
	this.getCharge = function() {
		var rest = 2 * (atom.bonds.length + atom.doublebonds.length + atom.triplebonds.length);
		var charge = 0;
		if (rest > 8) {
			// Over octet... possible if S or P atoms or even bigger atoms
			charge =
				ELEMENTS.getValenceElectron(myElem) -
				(atom.bonds.length + atom.doublebonds.length + atom.triplebonds.length);
		} else {
			charge =
				ELEMENTS.getValenceElectron(myElem) +
				atom.bonds.length +
	...
	this.assignFFType = function() {
		...
		if (self.fftype === undefined) {
			self.structure.LogError(
				new BioError(BIOERRORS.LOG_TO_CONSOLE, BIOERRORS.FF, [ self ], 'Force Field Type Unknown')
			);
			return;
		}
	...
	this.BuildImplicitH = function(pH) {
		...
		var distance = ELEMENTS.getCovalentRadius(atom.element) + ELEMENTS.getCovalentRadius('H');
		...
		if (bonds.length == 0) {
			...
			for (var i = 0; i < bonds.length - 1; i++) {
				...
				if (atomCenter.id == atom1.id) {
					atom1.structure.LogError(
						BIOERRORS.LOG_TO_ALERT,
						new BioError(
							BIOERRORS.BONDING,
							[ atomCenter, atom1 ],
							'FATAL!!! Atoms share same id, implying self-bonding: will cause error'
						)
					);
	...
	this.correctSP2Hybridization = function() {
		...
		if (self.deloc.length === 1) {
			if (self.element !== 'N') {
				// Nitrogen is SP2 by default because of tunneling of lone pair
				self.structure.LogError(
					new BioError(
			...
			if (self.element !== 'N' && self.element !== 'O' && self.hybridization == 'SP2') {
				self.structure.LogError(
					new BioError(
						BIOERRORS.LOG_TO_INTERNAL,
						BIOERRORS.HYBRIDIZATION,
	...
	this.addBond = function(type, atomID) {
		if (type === 'single') {
			...
			// the input type is wrong. only "single" or "double" accepted
			new BioError(
				BIOERRORS.LOG_TO_CONSOLE,
				BIOERRORS.BONDING,
				[ self ],
	...
	this.checkValence = function() {
		...
		if (period == 1) {
			...
			if (electrons > 8) {
				...
				if (charge == 0) {
					return 0;
				} else if (charge < 0 && self.element !== 'O') {
					return -1;
				} else if (charge < 0 && self.element === 'O') {
					return 0;
				} else if (charge > 0 && self.element !== 'N') {
					return 1;
				} else if (charge > 0 && self.element === 'N') {
					return 0;
				}
	...
	this.assignAromaticity = function() {
		...
		if (atom.inRing()[0] == false) {
			// not in a ring, not aromatic!
			atom.aromaticity = 0;
		} else if (atom.hybridization == 'SP3') {
			atom.aromaticity = 0;
		} else if (atom.deloc === null || atom.deloc.length == 0) {
			// In a ring but no delocalization
			atom.aromaticity = 0;
		} else {
			// in a ring and delocalized
			PIelec = 0;
	...
	this.assignImplicitHydrogens = function() {
		...
		} else if (ELEMENTS.getPeriod(self.element) > 2) {
			//For phosphorus or Sulfur
			//IMPLEMENT THIS??
		}
	};
	...
	this.printBonds = function() {
		var text = '';
		if (self.bonds == null || self.bonds.length == 0) {
			return 'Not Bonded';
		}
		for (var i = 0; i < self.bonds.length; i++) {
			text += self.bonds[i];
	...
	this.printDoubleBonds = function() {
		var text = '';
		if (self.doublebonds === null || self.doublebonds.length == 0) {
			return 'No Double Bonds';
		}
	...
	this.printDelocalization = function() {
		var text = '';
		if (self.deloc === null || self.deloc.length === 0) {
			return 'No Delocalization Group';
		}
		for (var i = 0; i < self.deloc.length; i++) {
			text += self.deloc[i];
	...
	this.printRingInfo = function() {
		var text = '';
		if (self.ring === null || self.ring.length == 0) {
	...
	this.printAromaticityInfo = function() {
		if (self.aromaticity === null) {
			return 'Not assigned yet';
		} else {
			if (self.isNonAromatic()) {
				return 'Non Aromatic';
			} else if (self.isAromatic()) {
				return 'Aromatic';
			} else if (self.isAntiAromatic()) {
	...
	this.printImplicitH = function() {
		var text = '';
		if (self.implicitH === null) {
			return 'Not yet assigned';
		}
		if (self.implicitH === 0) {
			return 'No Implicit Hydrogens';
		}
		text = self.implicitH;
		return text;
	};
	...
	this.printExplicitH = function() {
		var text = '';
		if (self.explicitH === null) {
			return 'Not yet assigned';
		}
		if (self.explicitH === 0) {
			return 'No Explicit Hydrogens';
		}
		text = self.implicitH;
		return text;
	};
	...
	this.checkAngles = function(buffer) {
		...
		if (bonds.length > 1) {
			for (var i = 0; i < bonds.length - 1; i++) {
				...
				if (atomCenter.id == atom1.id) {
					continue;
				} // THIS SHOULD NOT HAPPEN IF BONDS ARE PROPERLY ASSIGNED IN THE FIRST PLACE
				for (var x = i + 1; x < bonds.length; x++) {
					var atom3 = atoms[bonds[x]];
