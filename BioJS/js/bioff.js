...
function ForceField(ffparameters)
{
	...
	{
		if(Hatom.getFunctionalGroup() !== "Hb"){return;}
		var bestEnergy = 0;
		var bestAngle = 0;
...
function PhoenixHbondTerm()
{
	...
	{
		if(Hatom.element !== "H"){return [];}
		if(Hatom.bonds.length < 1){return [];}
	...
	{
		if(donor.hybridization === "SP2"
			&& acceptor.hybridization === "SP2")
		{
			...
			for(var i =0; i < bonds2.length; i++)
			{
				...
				if(otherAtom.id == Hatom.id){continue;}
			...
			if(atoms1.length >= 3 && atoms2.length >= 3)
			{
				return Math.abs(
						TorsionBetweenAtomPlanes(
								atoms1[0],atoms1[1],atoms1[2],
								atoms2[0],atoms2[1],atoms2[2],
								"radians"));
			}	
		}
		return null;
	...
	function ComputeAngleTerm(theta,phi,psi,acceptor,donor)
	{
		...
		else if(donor.hybridization === "SP3"
			&& acceptor.hybridization === "SP2")
		{
			F = Math.pow(Math.cos(theta),2)*Math.pow(Math.cos(phi),2);
		}
		else if(donor.hybridization === "SP2"
			&& acceptor.hybridization === "SP3")
		{
...
function PhoenixVdwTerm()
{
	...
	this.ComputeEnergy = function(atom)
	{
		...
		{
			if(near[i] == atom.id ||
					containsKey(atom.bonds,near[i])){continue;}
	...
	this.setScaleFactor=  function(num)
	{
		if(num < 0.8 || num > 1.10)
		{
			new BioERROR(BIOERRORS.FF,[self],"Careful!! Scaling Factor for Dreiding FF might be too low or too strong");
		}
...
{
	this.name = "dreiding";
	const Accepted = ["H","B","C","N","O","F","Na","Al","Si","Ga","Ge","Se","Sn","Te","Sb","As","In",
	                  "P","S","Cl","Br","I","Ca","Fe","Mn","Ti","Zn","Ru"];
	...
	const Params = 
					{
						...
						F_:		[0.3050,3.2850],
						Na:		[0.5000,3.1440],
						Mg:		[0.5000,2.9000], // added By @Author Olivier Gagnon. Not described in the FF (based on Mg covalent Radii (1.45A) and Na Well Depth (0.5_)
						Al3:	[0.0650,4.6150],
						Si3:	[0.0950,4.4350],
						Ga3:	[0.4000,4.3900],
						Ge3:	[0.4000,4.2700],
	...
	this.getType = function(atom)
	{
		...
		if(!containsKey(Accepted,element)){atom.structure.LogError(new BioError(
				BIOERRORS.LOG_TO_CONSOLE,
				BIOERRORS.FF,
				[self],
				"Cannot Assign Dreiding Atom Type, Element unrecognize"));}
		//In order of priorities
		else if(element === "H" && fg === "Hb"){return "H___HB";}
		else if(element === "H"){return "H_";}
		
		else if(element === "F"){return "F_";}
		else if(element === "Na"){return "Na";}
		else if(element === "Al" && hyb == "SP3"){return "Al3";}
		else if(element === "Si" && hyb == "SP3"){return "Si3";}
		else if(element === "Ga" && hyb == "SP3"){return "Ga3";}
		else if(element === "Ge" && hyb == "SP3"){return "Ge3";}
		else if(element === "As" && hyb == "SP3"){return "As3";}
		else if(element === "Se" && hyb == "SP3"){return "Se3";}
		else if(element === "In" && hyb == "SP3"){return "In3";}
		else if(element === "Sn" && hyb == "SP3"){return "Sn3";}
		else if(element === "Sb" && hyb == "SP3"){return "Sb3";}
		else if(element === "Te" && hyb == "SP3"){return "Te3";}
		else if(element === "P" && hyb == "SP3") {return "P_3";}
		else if(element === "S" && hyb == "SP3") {return "S_3";}

		else if(element === "Cl"){return "Cl";}
		else if(element === "I"){return "I_";}
		...
		//In order of priorities
		else if(element === "C" && deloc === true){return "C_R";}
		else if(element === "C" && hyb === "SP2"){ return "C_2";}
		else if(element === "C" && hyb === "SP") { return "C_1";}
		else if(element === "C" && hyb === "SP3"){ return "C_3";}
		
		else if(element === "N" && deloc === true){return "N_R";}
		else if(element === "N" && hyb === "SP2"){ return "N_2";}
		else if(element === "N" && hyb === "SP") { return "N_1";}
		else if(element === "N" && hyb === "SP3"){ return "N_3";}
		
		else if(element === "O" && deloc === true){return "O_R";}
		else if(element === "O" && hyb === "SP2"){ return "O_2";}
		else if(element === "O" && hyb === "SP3") { return "O_3";}
		else{
...
function FunctionalGroupHandler()
{
	...
	this.pKa = function(_atom)
	{
		...
		if(myElem == "C" || myElem == "H"){return null;}
		...
		for(var i = 0; i < fg.length; i++)
		{
			...
			var element = atom.element;
			if(element == "C"){cNum++;}
			else if(element == "N"){nNum++;}
			else if(element == "O"){oNum++;}
			else if(element == "P"){pNum++;}
			else if(element == "S"){sNum++;}
			
			if(atom.isAromatic()){aromatics += 1/atom.ring.length;};
			doublebonds += atom.doublebonds.length;
			if(atom.inRing()[0] == true){rings++;}
		}
		...
		if(
			...
			oNum 		=== 0 	&&
			pNum 		=== 0)
			{return PKAS.NCNH2NH2;} // guanidine
		else if(
			aromatics 	=== 1 	&&
			rings 		=== 5 	&&
			doublebonds === 4 	&&
			...
			oNum 		=== 0 	&&
			pNum 		=== 0)
			{return PKAS.CNHCNHC;} // histidine ring
		else if(
			aromatics 	=== 1 	&&
			rings 		=== 6 	&&
			doublebonds === 6 	&&
